...
func TestNewBuilderWithVariadicTxs(t *testing.T) {
	...
	t.Run("normal txs only", func(t *testing.T) {
		...
		require.NoError(t, err)
		builder, err := square.NewBuilder(64, 64, test.ClassifyNormalTxs(txs)...)
		require.NoError(t, err)
	...
	t.Run("fibre txs only", func(t *testing.T) {
		fibreTx := newFibreClassifiedTx(t, ns1)
		builder, err := square.NewBuilder(64, 64, fibreTx)
		require.NoError(t, err)
		require.Len(t, builder.Txs, 0)
		require.Len(t, builder.Pfbs, 0)
	...
	t.Run("mixed normal, blob, and fibre txs", func(t *testing.T) {
		...
		require.NoError(t, err)
		txs = append(txs, blobTxs...)
		classified := append(test.ClassifyNormalTxs(txs), newFibreClassifiedTx(t, ns1))
	...
	t.Run("not enough space", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = square.NewBuilder(2, 64, test.ClassifyNormalTxs(blobTxs)...)
		require.Error(t, err)
	...
	t.Run("equivalent to manual append", func(t *testing.T) {
		...
		require.NoError(t, err)
		fibreTx := newFibreClassifiedTx(t, ns1)
		classified := make([]square.ClassifiedTx, 0, len(normalTxs)+len(blobTxs)+1)
		classified = append(classified, test.ClassifyNormalTxs(normalTxs)...)
		classified = append(classified, test.ClassifyNormalTxs(blobTxs)...)
		classified = append(classified, fibreTx)

		// Build via variadic parameter
		builderVariadic, err := square.NewBuilder(64, 64, classified...)
		require.NoError(t, err)
		...
		added, err := builderManual.AppendFibreTx(fibreTx.FibreTx)
		require.NoError(t, err)
		require.True(t, added)
...
func TestBuilderFindTxShareRange(t *testing.T) {
	...
	builder, err := square.NewBuilder(128, 64, test.ClassifyNormalTxs(blockTxs)...)
	require.NoError(t, err)
...
func TestBigBlock(t *testing.T) {
	...
	builder, err := square.NewBuilder(defaultMaxSquareSize, defaultSubtreeRootThreshold, test.ClassifyNormalTxs(bigBlock.Txs)...)
	require.NoError(t, err)
